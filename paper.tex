...
This article proposes a fully described real-time framework designed to simulate the free surface of the ocean and the interactions between solid objects and the surrounding fluid in real time (\cf Figure \ref{fig:overviewFramework}).
...
\begin{itemize}
	...
	\item The simulation of the alteration of a free surface by a solid, inspired by Coords and Staadt works (2009) \cite{cordsRealTimeOpenWater2009}.
\end{itemize}
...
\begin{itemize}
	...
	\item An algorithm to compute the input of the Coords and Staadt method more realistically from the free surface.
\end{itemize}
...
However, to our knowledge, this type of method is either too expensive for real-time simulation or not general enough because it requires some physical pre-computation based on CFD or experimental data.
...
Gomez (2000) \cite{gomezInteractiveSimulationWater2000} solved the wave equation using the FDM to simulate waves on the free surface.
This method was improved by Coords and Staadt (2009) \cite{cordsRealTimeOpenWater2009} to work with moving objects.
...
For solid-fluid coupling, \framework based its calculations on the finite-difference method (FDM) described in Gomez (2000) \cite{gomezInteractiveSimulationWater2000} and its improvement by Coords and Staadt (2009) \cite{cordsRealTimeOpenWater2009}.
%-------------------------------------------------------------------------
\section{Ocean waves}
...
\begin{itemize}
	\item The scalar $H\in\mathbb R^+$ denotes the depth of the water. \Framework assumes \textbf{deep water} and therefore supposes that $H$ tends towards $+\infty$\footnote{\cf appendix \ref{sec:annexe} for more details on this assumption.}.
	...
	\item The vector $\phi(\mathbf x,y,t)\in\mathbb R^3$ is the potential velocity of fluid at the horizontal position $\mathbf x$, depth $y$ and time $t$.
	\item The scalar $g=9.80665~m.s^{-2}$ denotes the standard gravity, while $\mathbf g\in\mathbb R^3$ is the gravitational field.
\end{itemize}

Tessendorf method is based on the following approximations of the Navier-Stokes equations\footnote{\cf appendix \ref{sec:annexe-NS}.}:
\begin{equation}
...
\end{equation}
where $k$ is the Euclidean norm of the wave vector $\mathbf{k}$, $\omega(k)=\sqrt{gk}$ is the \textbf{dispersion relation}\footnote{The dispersion relation is defined for deep water, \cf appendix \ref{sec:annexe} for more details on this assumption.} and $\tilde{h}_0^*$ the conjugate of $\tilde{h}_0$.
...
The product between the Donelan-Banner and $\xi$ spectra is denoted by $D_{DB\xi}$, and is expressed as follows:
\begin{equation}
...
Finally, \framework uses a last parameter $\delta\in[0,1]$ that represents how much the waves orientation is constant ($\delta = 0$) or not ($\delta = 1$) (\cf Figure \ref{fig:dispersionEffect}).
...
\begin{figure}[htbp]
	...
	{
		\includegraphics[width=0.4\textwidth]{Figures/seaDispersion0SeaState5-contrast.png}
		\label{fig:dispersion0}
	}
	\hspace{1cm}
	\subfloat[Using $\delta = 1$.]
	{
		\includegraphics[width=0.4\textwidth]{Figures/seaDispersion1SeaState5-contrast.png}
...
Starting from the system \ref{equa:systemTessendorf}, \framework proposes to compute the velocity of the water at any depth $y$, as follows\footnote{\cf appendix \ref{sec:annexe} for more details on the origin of this expression}:
...
In \framework, the number of IFFT applications is divided by two using Theorem \ref{theorem:ifft} on Hermitian matrices\footnote{A matrix $X=(x_{n,m})$ is Hermitian if it is equal to its own conjugate transpose: $X = X^{\dagger}$ or $x_{n,m}=x^*_{m,n}$.} (\cf annex \ref{sec:annexe-2IFFT} for its demonstration).
\begin{theorem}
	\label{theorem:ifft}
...
To make $P(d)$ and $A(d)$ having the same order of magnitude, the units are the \textbf{decisecond} for performance and the \textbf{meter per seconds} for accuracy (\cf figure \ref{fig:interpolationCriteria}).
...
\begin{figure}[ht]
	...
	\caption{Multi-objective function $P$ to optimize, with two objectives: performance and accuracy.
		Its minimum is reached by the interpolation degree of $8$. For more details on each objective, \cf Figures \ref{fig:interpolationCriteria} which represent them individually.}
...
\Framework relies on the method developed by Coords and Staadt (2009) \cite{cordsRealTimeOpenWater2009} to simulate the action of the solid on the fluid.
...
\begin{equation}
	\label{eq:2dwave}
	\nabla h(\mathbf x,t)-\frac{1}{c^2}\frac{\partial^2 h(\mathbf x,t)}{\partial t^2}=0
\end{equation}
where $h(\mathbf x,t)$ is the height of the water surface at horizontal position $\mathbf x$ and time $t$, $\nabla=\frac{\partial^2}{\partial x^2}+\frac{\partial^2}{\partial z^2}$ is the Laplacian in 2D and $c$ is the wave velocity expressed in $m.s^{-1}$.
...
\begin{equation}
	d(t) =
	\begin{cases}
		\left(1-\frac{||v_M(t)||}{v_{max}}\right)d_0 + \frac{||v_M(t)||}{v_{max}}d_{max} & \text{for~~}||v_M(t)||\leq v_{max}, \\
		d_{max}                                                                          & \text{else}
	\end{cases}
\end{equation}
...
Hence, when $M$ moves, the waves have to move accordingly as done by Coords and Staadt (2009) \cite{cordsRealTimeOpenWater2009}.
...
To solve this problem, the size $m$ of the boundary of $Z$ is increased such that $m>1$ (\cf Figure \ref{fig:examGrid}).
...
\Framework uses a simple model that tries to reproduce parts of the idea of Coords and Staadt (2009) \cite{cordsRealTimeOpenWater2009}.
...
\begin{equation}
	h_f = V_{M}h_{M}\times i_f\times\frac{v_w}{v_{M}},
\end{equation}
where $V_M$ is the velocity of the body $M$, $h_M$ is the full height of $M$, $i_f$ is a user parameter allowing more user control, $v_w$ is the submerged volume of $M$ (\cf Section~\ref{sec:fluidToSolidAction}), and $v_{M}$ is the full volume of $M$.
...
Figure \ref{fig:mask} shows some views of the application of this interactive waves process for a simple rectangular boat.
...
\begin{figure}[htbp]
	...
	\subfloat[A theoretical view of the water height function $h(\mathbf x,t)$ that will be applied to a simple rectangle.]
	{
		\includegraphics[width=0.25\textwidth]{Figures/Mask3D_theoric_view.png}
		\label{subfig:maskTheoreticalView}
	}
	\hspace{1cm}
	\subfloat[The mask of a simple boat encoded in a single channel texture.]
	{
		\includegraphics[width=0.25\textwidth]{Figures/MaskHeightMap.png}
		\label{subfig:maskHeightMap}
	}
	\hspace{1cm}
	\subfloat[The mask of a simple boat in a 3D scene with a very low sea state.]
	{
		\includegraphics[width=0.25\textwidth]{Figures/Mask3D-contrast.png}
		\label{subfig:mask3D}
	}
	\caption{Three different views of the mask.}
	\label{fig:mask}
\end{figure}
...
\Framework is a demonstration that these methods achieve real-time performance on a desktop PC (\cf section \ref{sec:results_and_discussions}).
...
As mention in Section~3.3.2 of Hulin \etal (2001) \cite{hulinHydrodynamiquePhysique2001}, a fluid can be considered incompressible if $u<<c_{sound}$ where $c_{sound}$ if the sound speed in ocean which is approximately equal to $c_{sound} = 1530m.s^{-1}$\footnote{\cf \url{https://en.wikipedia.org/wiki/Sound_speed_profile}.}.
...
Then, there exists a scalar field called \textbf{velocity potential} defined by $\phi = \mathbf{\nabla} \phi$ from \emph{Helmholtz decomposition}\footnote{\cf \url{https://en.wikipedia.org/wiki/Helmholtz_decomposition}.}.

These three assumptions lead to the Bernoulli equation (\cf Section~5.3.2 of Hulin \etal (2001), which is the following simplified version of the Cauchy momentum equation:
\begin{equation}
