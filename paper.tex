...
\begin{figure}[!h]
	...
	\caption{Summary of forces calculation. Using the same notation as Section~\ref{subsec:forces} it follows:
		the sum of the buoyancy forces \textcolor{jauneC}{$F_b$} in \textcolor{jauneC}{yellow}, applied to the center of immersion;
...
The optimal number of iterations ($N_{\text{iter}}$) required for the iterative retrieval of the water height was determined through a series of tests. Wind speeds were varied from $0.1 \, \text{m/s}$ to $35.0 \, \text{m/s}$ in increments of $0.5 \, \text{m/s}$, and the convergence of water height was evaluated at $1000$ random positions over a $10^4 \, \text{m} \times 10^4 \, \text{m}$ domain.

The program iteratively computed the water height until the changes in height were smaller than a $1$ cm.

The results, as shown in Figure~\ref{fig:convergence_steps}, demonstrate that the required number of iterations increases with wind speed, stabilizing at approximately $N_{\text{iter}} = 4$ for higher wind speeds. This finding suggests that using $N_{\text{iter}} = 4$ ensures reliable convergence for all tested conditions while balancing computational efficiency. 
